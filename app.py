...
# ========================================================
# STEP 2: DATA FETCHING (Fixed for Column E)
# ========================================================
@st.cache_data(ttl=0) # Har baar naya data uthayega
def load_all_data():
    import time
    # 't' parameter add karne se cache bypass ho jata hai
    t = int(time.time())
    base = f"https://docs.google.com/spreadsheets/d/{SHEET_ID}/gviz/tq?tqx=out:csv&t={t}&sheet="
    
    # Users data load karein aur ensure karein ke column E shamil ho
    u_df = pd.read_csv(base + "Users")
    u_df.columns = [str(c).strip() for c in u_df.columns] # Column names saaf karein
