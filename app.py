...
SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwOKzymo0Cl84EM5nCnwg3KnCSLuO6jI6CrvRI6GCXI7-vMVPTMQ2RFgTnbXj18jAsY/exec"
...
def get_data():
    o = pd.read_csv(f"{ORDERS_URL}&cache={datetime.now().timestamp()}", dtype={'Phone': str}).fillna('')
    s = pd.read_csv(f"{SETTINGS_URL}&cache={datetime.now().timestamp()}").fillna('')
    return o, s

orders_df, settings_df = get_data()
...
# ---------------- DASHBOARD ----------------
    ...
    if menu == "👤 My Profile":
        ...
        st.markdown("""<style>.p-block { background-color: #f8f9fa; padding: 15px; border-radius: 12px; border-left: 6px solid #007bff; margin-bottom: 12px; }</style>""", unsafe_allow_html=True)
        st.markdown(f"""<div class="p-block" style="background-color: #e3f2fd; border-left-color: #2196f3;"><b>👤 User Profile</b><br>{st.session_state.user_data['Name']}<br>{st.session_state.user_data['Phone']}</div>""", unsafe_allow_html=True)
        
        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"""<div class="p-block" style="background-color: #fff9c4; border-left-color: #fbc02d;">🪙 Points: {pts:.1f}</div>""", unsafe_allow_html=True)
            st.markdown(f"""<div class="p-block" style="background-color: #c8e6c9; border-left-color: #4caf50;">📦 Orders: {len(u_orders)}</div>""", unsafe_allow_html=True)
        with c2:
            st.markdown(f"""<div class="p-block" style="background-color: #ffccbc; border-left-color: #ff5722;">⏳ Pending: {len(u_orders[u_orders['Status'].str.contains("Order", na=False)])}</div>""", unsafe_allow_html=True)
            st.markdown(f"""<div class="p-block" style="background-color: #e1f5fe; border-left-color: #03a9f4;">🏆 Account: Verified</div>""", unsafe_allow_html=True)
    ...
    # --- 🛍️ ORDER NOW ---
        ...
        sel_prod = c2.selectbox("Product", prods['Product Name'].unique())
        ...
        if st.session_state.cart:
            ...
            if st.button(f"Confirm Order (Rs. {bill})"):
                ...
                pdf = generate_pdf(st.session_state.user_data['Name'], st.session_state.user_data['Phone'], items_str, bill, "Thermal")
                st.download_button("Download Receipt 📄", data=pdf.getvalue(), file_name="receipt.pdf", mime="application/pdf")
                st.session_state.cart = []

    # --- 🔐 ADMIN DASHBOARD ---
    elif menu == "🔐 Admin Dashboard":
        st.header("Admin Management")
        tab1, tab2 = st.tabs(["Active Orders", "All Data"])
        
        with tab1:
            active = orders_df[orders_df['Status'].str.contains("Order", na=False)]
            for idx, row in active.iterrows():
                with st.expander(f"{row['Name']} - Rs.{row['Bill']}"):
                    ...
                    c3.download_button("A4", a4.getvalue(), f"A4_{idx}.pdf")
                    th = generate_pdf(row['Name'], row['Phone'], row['Product'], row['Bill'], "Thermal")
                    c4.download_button("Thermal", th.getvalue(), f"TH_{idx}.pdf")
